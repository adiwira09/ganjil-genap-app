...
def load_sqlite_data(frame, interval=1000):
    # Inisialisasi Treeview
    columns = ["Track ID", "Text Plate", "Date", "Keterangan"]
    tree = ttk.Treeview(frame, columns=columns, show="headings")
    ...
    def update_sqlite_data():
        """Membaca ulang data dari SQLite dan memperbarui Treeview."""
        rows = db.read_db()
        ...
        # Tambahkan data baru ke Treeview
        for row in reversed(rows):  # Tampilkan data terbaru terlebih dahulu
            tree.insert("", "end", values=row)

        # Jadwalkan pembaruan berikutnya
        frame.after(interval, update_sqlite_data)
